...
export default function VideoCall({
  ...
  const hasIncomingRequest = Boolean(incomingOffer);
  const showVideoStage = !compact || isCallActive || hasIncomingRequest;

  const statusLabel = useMemo(() => {
    if (callError) return callError;
    if (hasIncomingRequest) return `${incomingFrom || peerLabel} is calling`;
  ...
  }, [callError, hasIncomingRequest, incomingFrom, peerLabel, callStatus, callSeconds, canCall]);
  ...
  const createPeerConnection = async () => {
    ...
    connection.ontrack = (event) => {
      ...
      setHasRemotePreview(targetStream.getTracks().length > 0);
    };
  ...
  const acceptCall = async () => {
    if (!incomingOffer || !socket || !signalingRoomId) return;
  ...
  return (
    <section className={`vc-panel ${compact ? 'vc-panel-compact' : ''}`}>
